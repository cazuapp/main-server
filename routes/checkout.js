...
router.post('/set_status', [VLR.NeedsID, VLR.NeedsKey, Checkout.CanCheck], async function (req, res) 
{
    Checkout.SetStatus(req).then(function (result) 
    {
...
router.post('/status', [VLR.NeedsID], async function (req, res) 
{
    Checkout.GetStatus(req).then(function (result) 
    {
        res.status(httpr.ok).send({ data: result });
    })
    .catch(function (error) 
